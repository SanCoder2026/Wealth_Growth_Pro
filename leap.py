...
def analyze_leap_lag(ticker, dummy_mode=False, dummy_date=None, dummy_time=None):
    try:
        if dummy_mode and dummy_date and dummy_time:
            ...
            from_date = (target_dt - timedelta(days=7)).strftime("%Y-%m-%d")
...
st.caption("LEAPs Lag Hunter • Powered by Polygon.io • Top 5 only • Full reasoning • Budget optimized")            price_later = float(hist['Close'].iloc[later_idx])
            move_pct = (price_later - current_price) / current_price * 100
        else:
            # Live Mode
