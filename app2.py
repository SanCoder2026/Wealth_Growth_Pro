...
with st.expander("💰 Capital Management", expanded=False):
    ...
    with col_cap1:
        ...
        if st.button("Set Initial Capital (can only do once)"):
            new_initial = st.number_input("Initial Capital Amount", min_value=0.0, value=initial_capital, step=1000.0, key="set_initial")
    ...
    with col_cap2:
        ...
        add_amount = st.number_input("Amount to Add ($)", min_value=0.0, step=1000.0, key="add_capital")
...
with st.expander("📈 Manage Tickers & Allocations", expanded=False):
    ...
    new_ticker = st.text_input("Enter Ticker Symbol (e.g., NVDA, TSLA)", key="new_ticker").strip().upper()
    ...
    target_sum = sum(d["target_pct"] for d in etfs.values())
    if abs(target_sum - 1.0) > 0.001:
        st.warning(f"Total target allocation is {target_sum*100:.1f}% (should be 100%). Values will be normalized on save.")
    ...
    for t in list(etfs.keys()):
        ...
        with col_t1:
            st.write(t)
        with col_t2:
            new_pct = st.number_input(f"Target % for {t}", min_value=0.0, max_value=100.0, value=etfs[t]["target_pct"]*100, step=0.1, key=f"tgt_{t}")
            etfs[t]["target_pct"] = new_pct / 100

    if st.button("Save & Normalize Targets"):
        current_sum = sum(d["target_pct"] for d in etfs.values())
        if current_sum > 0:
            for t in etfs:
...
with st.expander("📊 Data Entry (Expand to update – Collapse when done)", expanded=True):
    ...
    with col_left:
        ...
        if st.button("Suggest Reinvestment") and premium > 0:
            ...
            deviations = {t: total_val * etfs[t]["target_pct"] - (etfs[t]["shares"] * prices.get(t, 0)) for t in etfs}
...
if history:
    ...
    fig.update_layout(height=600, title="Path to $1M")
