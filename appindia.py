...
# === CONFIG ===
st.set_page_config(page_title="Wealth Growth Pro → ₹1 Cr", layout="wide", initial_sidebar_state="expanded")
PREMIUM_TARGET_MONTHLY = 100000.0  # in INR

# Attractive centered title - Indian version
st.markdown(
    """
    ...
    <p style='text-align: center; color: #444; font-size: 1.4rem; margin-top: -15px; margin-bottom: 30px;'>
        → Systematic Growth to ₹1 Crore with Discipline & Strategy
    </p>
...
# No pre-filled TARGET_ALLOCATIONS — start empty
...
def load_latest():
    ...
    return {
        "etfs": {},  # start empty — user adds tickers
        "history": [],
        "initial_capital": 0.0,
        "capital_additions": [],
        "option_trades": []
...
# === VOLATILITY / PREMIUM POTENTIAL ESTIMATOR ===
@st.cache_data(ttl=3600)
def estimate_premium_potential(ticker):
    try:
        stock = yf.Ticker(ticker)
        hist = stock.history(period="3mo")

        if len(hist) < 20:
            return {"score": 10, "suggested_pct": 0.03, "reason": "Insufficient history"}

        daily_ret = hist['Close'].pct_change().dropna()
        hist_vol = daily_ret.std() * (252 ** 0.5) * 100  # annualized %

        recent = hist.tail(5)
        range_pct = (recent['High'].max() - recent['Low'].min()) / recent['Close'].iloc[-1] * 100

        nifty = yf.Ticker("^NSEI").history(period="3mo")['Close'].pct_change().dropna()
        if len(nifty) >= len(daily_ret):
            cov = np.cov(daily_ret[-len(nifty):], nifty)[0,1]
            var = np.var(nifty)
        ...
        if score < 25:
            suggested = 0.04
            reason = "Low volatility – modest premium potential"
        elif score < 45:
            suggested = 0.10
            reason = "Moderate volatility – decent weekly premium"
        elif score < 65:
            suggested = 0.20
            reason = "Good volatility – strong premium generator"
        else:
            suggested = 0.28
            reason = "High volatility – excellent premium potential (core position)"

        return {"score": round(score, 1), "suggested_pct": suggested, "reason": reason}

    except:
        return {"score": 10, "suggested_pct": 0.03, "reason": "Data not available"}
...
def fetch_prices(tickers_list):
    ...
    for t in tickers_list:
        try:
            symbol = t if t.endswith((".NS", ".BO")) else t + ".NS"
            data = yf.Ticker(symbol)
            price = data.info.get('currentPrice') or data.info.get('regularMarketPrice', 0)
            prices[t] = round(price, 2) if price else 0
        except:
            prices[t] = 0
    return prices
...
pct_to_m = max(0, (net_equity / 10000000) * 100) if net_equity > 0 else 0  # ₹1 Cr
...
col5.metric("Progress to ₹1 Cr", f"{pct_to_m:.2f}%")
...
with st.expander("💰 Capital & Margin"):
    ...
    with col1:
        ...
        add_amount = st.number_input("Amount (₹)", min_value=0.0, step=10000.0, key="add_cap")
    ...
    with col2:
        ...
        margin_input = st.number_input("Current Margin (₹)", min_value=0.0, value=float(margin), step=10000.0, format="%.2f")
...
with st.expander("📈 Manage Tickers & Rebalance", expanded=True):
    st.subheader("Add New Ticker (NSE: .NS | BSE: .BO)")
    col1a, col1b = st.columns([3, 2])

    with col1a:
        new_ticker = st.text_input("Ticker Symbol", "", key="add_new_tkr", placeholder="e.g. RELIANCE.NS, NIFTYBEES.NS").strip().upper()

    if new_ticker:
        if new_ticker in etfs:
            st.warning(f"{new_ticker} already exists in your portfolio.")
        else:
            est = estimate_premium_potential(new_ticker)

            st.info(f"**Premium earning potential estimate**\n\n"
                    f"Volatility Score: **{est['score']}/100**\n"
                    f"Suggested target allocation: **{est['suggested_pct']*100:.0f}%**\n"
                    f"_{est['reason']}_")

            if st.button(f"Add {new_ticker} → Apply Suggested Allocation", type="primary"):
                etfs[new_ticker] = {
                    "shares": 0.0,
                    "cost_basis": 0.0,
                    "target_pct": est['suggested_pct'],
                    "contracts_sold": 0,
                    "weekly_contracts": 0
                }
                save_version({
                ...
                st.success(f"Added **{new_ticker}** with suggested **{est['suggested_pct']*100:.0f}%** target")
                st.rerun()

# === OPTIONS / WHEEL SECTION ===
with st.expander("🛞 Options Trading & Weekly Wheel (Paper)"):
    st.subheader("Sell Weekly Call")
    ticker = st.selectbox("Ticker", list(etfs.keys()) if etfs else ["No tickers yet"], key="opt_tkr")
    if ticker and ticker in etfs and prices.get(ticker, 0) > 0:
        ...
        otm_pct = 0.12 if "BANK" in ticker or "NIFTY" in ticker else 0.09
        strike = round(current_price * (1 + otm_pct), 2)

        st.write(f"Price: **₹{current_price:,.2f}**   |   Next Expiry: **{expiry_str}**")
        st.write(f"Suggested OTM Call Strike: **₹{strike:,.2f}**")

        contracts = st.number_input("Contracts to Sell", min_value=0, step=1, value=int(etfs[ticker].get("weekly_contracts", 0)))
        premium_per = st.number_input("Premium per contract (₹)", min_value=0.0, step=0.05)

        if st.button("Sell Calls"):
            if contracts <= 0:
                st.warning("Select at least 1 contract.")
            elif premium_per <= 0:
                st.warning("Enter positive premium.")
            else:
                ...
                option_trades.append({
                    ...
                    "contracts": int(contracts), "premium": float(premium_per), "status": "open"
                })
                save_version({
                    "etfs": etfs,
                    "history": history,
    ...
    if st.button("Run Assignment Check"):
        ...
        for trade in open_trades:
            ...
            if st.checkbox("Was assigned?", key=key):
                ...
                st.write("→ Next: consider selling put / call")
...
for t in sorted(etfs.keys()):
    ...
    profit_rupee = current_value - purchase_value
    profit_pct    = (profit_rupee / purchase_value * 100) if purchase_value > 0 else 0
    
    otm_pct = 0.10 if "BEES" in t or "ETF" in t else 0.12
    delta_est = "~30Δ"
    ...
    rows.append({
        ...
        "Profit ₹": f"₹{profit_rupee:,.2f}" if purchase_value > 0 else "-",
...
with st.expander("📊 Manual Updates"):
    ...
    with col_l:
        ...
        tk = st.selectbox("Ticker", list(etfs.keys()) if etfs else ["No tickers yet"], key="buy_tk")
        sh = st.number_input("Shares", 0.0001, step=0.0001)
        pr = st.number_input("Avg Price (₹)", 0.01, step=0.01)
        if st.button("Submit Buy") and tk != "No tickers yet":
            if sh > 0 and pr > 0:
                old = etfs[tk]
    ...
    with col_r:
        ...
        premium = st.number_input("Premium Received (₹)", 0.0, step=100.0)
...
if history:
    ...
    fig.add_hline(y=10000000, line_dash="dash", annotation_text="₹1 Cr Goal")
    fig.update_layout(height=550)
    st.plotly_chart(fig, use_container_width=True)

st.caption("Indian version — empty start — volatility-based allocation suggestions — NSE/BSE via yfinance")