...
if "option_trades" not in st.session_state:
    st.session_state.option_trades = []  # {"ticker": str, "type": "call"/"put", "strike": float, "expiry": str, "contracts": int, "premium": float, "status": "open"}
...
with st.expander("Options Trading & Wheel Strategy (Paper)", expanded=True):
    ...
    if ticker:
        ...
        if current_price == 0:
            ...
            default_contracts = max(1, etfs[ticker]["weekly_contracts"])
            contracts = st.number_input("Number of Call Contracts to Sell", min_value=1, step=1, value=default_contracts)
    ...
    if st.button("Run Monday Routine (Check assignments & next action)"):
        ...
        if not open_trades:
            ...
            for trade in open_trades:
                ...
                assigned = st.checkbox(f"Was this {trade['type']} assigned?", key=f"assign_{trade['ticker']}_{trade['expiry']}")
                if assigned:
                    trade["status"] = "assigned"
                    if trade["type"] == "call":
                        st.write("Call assigned — stocks called away")
                        st.write("→ Suggest: Sell put at same strike next week")
                    else:
...
# (same as previous version — omitted for brevity, include your full chart code here)
